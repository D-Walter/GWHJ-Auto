...
def dialogueTrans(matched):
    s = matched.group(1)
    res = s
    for c in globalsettings.known_dict:
        if s in globalsettings.known_dict[c]:
            res = globalsettings.known_dict[c][s]
    return "''" + res + "''"
        ...
        if mode == 'c':
            ...
            if match is not None:
                ...
                for _id in ids:
                    ...
                    print('r_name', r_name)
                    globalsettings.huiji_manager.save_new_page(r_name, '#重定向 [[' + name + ']]', summary='bot')
            globalsettings.huiji_manager.save_new_page(f'技能/{name}', content)
            globalsettings.huiji_manager.delete_page(name)
        else:
            print(content)

    normal_pages_process(globalsettings.get_path(['wikiText', 'zh', 'skill']), standardize_page, handle)
